...
// ParseTransferEvent provides a mock function with given fields: txHash, block_number, timestamp, call
func (_m *MockParser) ParseTransferEvent(txHash string, block_number uint64, timestamp uint64, call pkgtypes.CallFrame) []interface{} {
	ret := _m.Called(txHash, block_number, timestamp, call)

	if len(ret) == 0 {
		panic("no return value specified for ParseTransferEvent")
	}

	var r0 []interface{}
	if rf, ok := ret.Get(0).(func(string, uint64, uint64, pkgtypes.CallFrame) []interface{}); ok {
		r0 = rf(txHash, block_number, timestamp, call)
	} else {
		if ret.Get(0) != nil {
...
// UseTraceCall provides a mock function with no fields
