...
// Exchange provides a mock function with no fields
...
// GetEmptyPrice provides a mock function with given fields: limit
func (_m *MockStorage) GetEmptyPrice(limit uint64) ([]types.TradeLog, error) {
	ret := _m.Called(limit)

	if len(ret) == 0 {
		panic("no return value specified for GetEmptyPrice")
	}

	var r0 []types.TradeLog
	var r1 error
	if rf, ok := ret.Get(0).(func(uint64) ([]types.TradeLog, error)); ok {
		return rf(limit)
	}
	if rf, ok := ret.Get(0).(func(uint64) []types.TradeLog); ok {
		r0 = rf(limit)
	} else {
		if ret.Get(0) != nil {
			r0 = ret.Get(0).([]types.TradeLog)
...
// ResetTokenPriceToRefetch provides a mock function with given fields: token, from, to
func (_m *MockStorage) ResetTokenPriceToRefetch(token string, from int64, to int64) (int64, error) {
	ret := _m.Called(token, from, to)

	if len(ret) == 0 {
		panic("no return value specified for ResetTokenPriceToRefetch")
	}

	var r0 int64
	var r1 error
	if rf, ok := ret.Get(0).(func(string, int64, int64) (int64, error)); ok {
		return rf(token, from, to)
	}
